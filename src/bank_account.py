...
class BankAccounts:
    def __init__(self, property_file_path: str):
        ...
        self.bank_accounts = self._read_bank_accounts()

    def _read_bank_accounts(self):
        with open(self.json_file_path, "r") as file:
            return json.load(file)

    def get_bank_account(self, bank_account_code: str) -> dict:
        bank_account = self.bank_accounts["bankAccounts"].get(bank_account_code)
        if bank_account:
            return bank_account
        else:
...
class BankAccount:
    def __init__(self, bank_accounts: BankAccounts, bank_account_code: str):
        ...
        self.properties = bank_accounts.get_bank_account(bank_account_code)
